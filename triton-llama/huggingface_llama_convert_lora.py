...
def split_and_convert(args):
    ...
    tokenizer = LlamaTokenizer.from_pretrained(args.in_llama_file)
    ...
    for name, param in model.named_parameters():
        if name == LAYER_PREFIX+'model.embed_tokens.weight' or name == LAYER_PREFIX+'model.embed_tokens.modules_to_save.default.weight':
            param.detach().cpu().numpy().astype(np_weight_data_type).tofile(saved_dir + "model.wte.weight.bin")
            print(f"done layer model.wte.weight.bin", name)
        elif name == LAYER_PREFIX+'model.norm.weight':
            param.detach().cpu().numpy().astype(np_weight_data_type).tofile(saved_dir + "model.final_layernorm.weight.bin")
            print(f"done layer model.final_layernorm.weight.bin", name)
        elif name == LAYER_PREFIX+'lm_head.weight' or name == LAYER_PREFIX+'lm_head.modules_to_save.default.weight':
            param.detach().cpu().numpy().astype(np_weight_data_type).tofile(saved_dir + "model.lm_head.weight.bin")
            print(f"done layer model.lm_head.weight.bin", name)
