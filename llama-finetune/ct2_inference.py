...
def generate_prompt(instruction, input=None, data_name='alpaca'):
    if data_name == "alpaca":
        PROMPT_DICT = {
            "prompt_input": (
    ...
    elif data_name == "wizard":
        WIZARD_PROMPT = {
            "input": "A chat between a curious user and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the user's questions. USER: {instruction} ASSISTANT: "}
        return WIZARD_PROMPT['input'].format_map({"instruction": instruction})
    elif data_name == "":
        PROMPT = {"input": "{instruction}\n\n### Response:"}
        return PROMPT['input'].format_map({"instruction": instruction})
    else:
        raise ValueError("需要自己实现prompt")
